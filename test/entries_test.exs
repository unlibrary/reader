defmodule EntriesTest do
  ...
  test "list/1 lists entries for source & account" do
  ...
  test "list/1 skips read items" do
    {:ok, user} = Accounts.create("robijntje", valid_password())
    {:ok, source} = Sources.new(valid_feed_url(), :rss)
  ...
  test "list_all/1 doens't skip read items" do
    {:ok, user} = Accounts.create("robijntje", valid_password())
    {:ok, source} = Sources.new(valid_feed_url(), :rss)
    ...
    source_entries = Entries.list_all(source)
    assert length(source_entries) == 5

    {:ok, user} = Sources.add(source, user)
    user_entries = Entries.list_all(user)
